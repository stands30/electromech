...
$(document).ready(function()
{
   $('#piv_cmp_id').select2({
    ...
    ajax: {
      url: function(){
        return baseUrl+'inventory/getCompanyDropdown/?dispatch_order=1';
      },
      dataType: 'json',
    }
  });
   ...
   $('#piv_type_id').select2({
    ...
    ajax: {
      url: function(){
        return baseUrl+'inventory/getDODropdown/?cmp_id='+$('#piv_cmp_id').val();
      },
      dataType: 'json',
    }
  }).change(function(){
...
function getDOProducts()
{
  ...
  $.ajax(
        {
            type: "POST",
            url: baseUrl + "inventory/getDOProducts/"+piv_type_id+'?piv_location='+piv_location,
            dataType: "json",
            success: function(response)
            {
                 ...
                 for (i = 0; i < response.length; i++) 
                 {
                   ...
                   var prodSizeSelect2Val =
                     ...
                     if(response[i]["unblocked_stock"] == '')
                        ...
                        {
                          $('input[name="product_repeater_item['+i+'][piv_qty]"]').val(response[i]["piv_pending"]).addClass('edited');
                          $($('.piv_pending_span')[i]).html(response[i]["piv_pending"]+' / '+response[i]["unblocked_stock"]);
                          $($('.piv_pending_span')[i]).attr('data-piv_total',response[i]["piv_pending"]);
                          $($('.piv_total_span')[i]).html(response[i]["total_stock"]);
                        }
                      checkPendingValue($('select[name="product_repeater_item['+i+'][piv_prd_id_value]"]'));
                  }
                  assignCustomEvents();
